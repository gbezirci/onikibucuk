...
(function ($) {
  ...
  //   $(".services_wwd.feature__list").animate({
  //     scrollLeft: "+=150"  // 100 pixel sola kaydırır
  //   }, "slow");
  ...
  $(document).ready(function () {
    function checkScrollPosition() {
      var $div = $('.services_wwd.feature__list');
      var contentWidth = $div[0].scrollWidth;   // İçeriğin toplam genişliği
      var divWidth = $div.outerWidth();        // Div'in görünür genişliği
      var scrollLeft = $div.scrollLeft();      // Şu anki scroll pozisyonu

      // Eğer en baştaysak sol butonu gizle, değilse göster
      if (scrollLeft <= 0) {
        $('.prev-btn').hide();
      } else {
        $('.prev-btn').show();
      ...
      // Eğer en sonda isek sağ butonu gizle, değilse göster
      if (scrollLeft + divWidth >= contentWidth) {
        $('.next-btn').hide();
      } else {
        $('.next-btn').show();
      }
    ...
    // Div scroll edildiğinde kontrolü tekrar yap
    $('.services_wwd.feature__list').on('scroll', checkScrollPosition);

    $('.prev-btn').click(function () {
      $('.services_wwd.feature__list').animate({
        scrollLeft: "-=100"
      }, "slow", checkScrollPosition);  // Animasyon tamamlandığında kontrolü tekrar yap
    });
    ...
    $('.services_wwd.feature__list')
        ...
        .mouseleave(function() {  // Div dışına çıkıldığında da mouseup olarak kabul ediyoruz
            isMouseDown = false;
            $(this).css('cursor', 'grab');
        });
